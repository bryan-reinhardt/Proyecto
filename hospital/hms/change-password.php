...
{
$sql=mysqli_query($con,"SELECT password FROM  tblpatient where password='".md5($_POST['cpass'])."' && id='".$_SESSION['id']."'");
$num=mysqli_fetch_array($sql);
if($num>0)
{
 $con=mysqli_query($con,"update tblpatient set password='".md5($_POST['npass'])."', updationDate='$currentTime' where id='".$_SESSION['id']."'");
...
<html lang="en">
	<head>
		<title>Paciente | Cambio de Contraseña</title>
...
{
alert("¡Nueva contraseña archivada está vacía!");
document.chngpwd.npass.focus();
return false;
}
else if(document.chngpwd.cfpass.value=="")
{
alert("¡Confirme que la contraseña archivada esté vacía!");
...
<div class="form-group">
									...
									<input type="password" name="cfpass" class="form-control"  placeholder="****">
														</div>
